...
- **Agent Prompt Architecture** - Migrated all 12 base agent prompts from hardcoded TypeScript constants to dynamic loading from markdown files
  ...
  - Moved utility to `src/droids/utils.ts` to avoid circular dependency issues
  - Affected agents: planner, architect, executor, explore, researcher, designer, writer, qa-tester, vision, critic, analyst, scientist
- **Agent Prompt Enhancement** - Enhanced agent prompts with improved instructions and domain-specific guidance. Prompts are now maintained as standalone markdown files in `droids/`.
...
Fixed MCP server not working in Factory Droid plugin cache.
...
- **Updated Plugin Manifest** (`.factory-plugin/plugin.json`)
  - Added `mcpServers` reference to `.mcp.json`

### Changed

- Added `@modelcontextprotocol/sdk` as dependency
- Updated `package.json` files array to include `.factory-plugin` and `.mcp.json`
...
- **Agent Definitions** (`src/droids/definitions.ts`)
...
- **fix(subagent-tracker):** Replace CPU busy-wait loops with syncSleep (#159)
  ...
  - Complete state path migration to `.omd/state/` across all hooks and templates
...
- **fix(omd-setup):** Use cross-platform date conversion for state age check (#151)
...
- **Rate Limit Monitor** (`src/features/rate-limit-wait/`)
  ...
  - tmux pane detection for blocked Factory Droid sessions
  - Background daemon that polls and auto-resumes when limit clears
  - CLI commands: `omd wait status`, `omd wait daemon start/stop`, `omd wait detect`
...
- **Smart CLI** (`src/cli/commands/wait.ts`)
  - Zero learning curve: `omd wait` shows status and suggests next action
  - `omd wait --start` / `--stop` shortcuts
  - 61 tests covering real-world scenarios

#### Async Hook Lifecycle Modules (5 New Hooks)
Factory Droid integration hooks for session and tool lifecycle events.
...
- **Smart Agent Suggestions**
  ...
  - Audit logging to `.omd/logs/delegation-audit.jsonl`
...
**Configuration:**
- Local: `.omd/config.json` → `delegationEnforcementLevel`
- Global: `~/.factory/.omd-config.json` → `delegationEnforcementLevel`

## [3.6.3] - 2026-01-27

### Fixed

- Fix npm package references (`oh-my-droid` → `oh-my-droid`) in docs/MIGRATION.md, commands/doctor.md, skills/doctor/SKILL.md
...
- **PSM Skill** (`skills/psm/`)
  - `/oh-my-droid:psm` command for project session management
  - Automatic FACTORY.md and AGENTS.md injection on session start
  - Project isolation with dedicated session contexts
  - Session persistence and resume support
  - Integration with existing oh-my-droid workflows
...
- **Worker Preamble Protocol** (`src/droids/preamble.ts`)
...
All execution mode state files consolidated into `.omd/state/` subdirectory:
...
- `src/droids/preamble.ts` - Worker preamble protocol
...
### Added
- feat(skills): add learn-about-omd skill for usage pattern analysis
...
- **Transcript Scanner** (`src/analytics/transcript-scanner.ts`)
  - Scans `~/.factory/projects/` for session JSONL files
...
- **Backfill Deduplication** (`src/analytics/backfill-dedup.ts`)
  ...
  - Persistent state in `~/.omd/state/backfill-state.json`
  - Prevents duplicate entries on re-runs

- **CLI Command** (`omd backfill`)
  - Progress bar with real-time stats
  - Options: `--project`, `--from`, `--to`, `--dry-run`, `--verbose`
  - Auto-backfill on first `omd stats` run

- **Global Token Tracking**
  - All token data stored globally in `~/.omd/state/` (not per-project)
...
- **Critical**: Updated `.factory-plugin/plugin.json` version to 3.4.2
  - Required for Factory Droid plugin system to recognize the latest version
...
- **Website Repository**:
  - Updated oh-my-droid-website to v3.4.0
...
- **Ultrapilot**: Parallel autopilot with up to 5 concurrent workers
  ...
  - State tracking in `.omd/state/ultrapilot-state.json` and `.omd/state/ultrapilot-ownership.json`
...
- **Ecomode**: Token-efficient parallel execution mode
  ...
  - State tracking in `.omd/ecomode-state.json`
...
- **State Management Module** (`src/features/state-manager/`)
  - Standardized state locations: `.omd/state/{name}.json` (local), `~/.omd/state/{name}.json` (global)
...
- **Agent Prompt Template System** (`droids/templates/`)
...
- **QA-Tester-High Prompt**: Extracted inline prompt to `droids/qa-tester-high.md`
...
### Added
- **omd-setup**: GitHub star prompt after setup completion (#82)
...
### Added
- MCP server configuration skill (`/oh-my-droid:mcp-setup`) for Context7, Exa, Filesystem, GitHub (#74)
- MCP setup integrated into omd-setup wizard
...
- **AskUserQuestion Tool Enforcement in Plan Mode** (#70): Planner now uses the `AskUserQuestion` tool for preference questions, allowing users to select options via clickable UI instead of typing responses
  - Added MANDATORY section to `droids/planner.md` with examples and question type classification
  - Added global reminder to `docs/FACTORY.md` in Internal Protocols
...
- **Research Command** (`/oh-my-droid:research`): Orchestrate parallel scientist agents for complex research
...
- `src/droids/scientist.ts` - Scientist agent definition
- `droids/scientist.md`, `scientist-low.md`, `scientist-high.md` - Agent prompts
...
- **PreToolUse hooks**: Soft enforcement for delegation via `pre-tool-use.sh` and `pre-tool-use.mjs`
  ...
  - Path-based exceptions for `.omd/`, `.factory/`, `FACTORY.md`, `AGENTS.md`
...
### Added
- **Autopilot Command** (`/oh-my-droid:autopilot`): Full autonomous execution from idea to working code
...
- **Cancel-autopilot Skill** (`/oh-my-droid:cancel-autopilot`): Graceful cancellation with state preservation
...
- Updated docs/FACTORY.md with autopilot integration
...
- **Update notification**: omd-setup now checks for and notifies about available updates
...
### Changed
- **omd-setup rebuild behavior**: Now shows version changes when refreshing FACTORY.md
- **Cache clearing**: omd-setup automatically removes stale plugin cache versions
...
- **Fully-qualified command names**: Documentation now uses `/oh-my-droid:omd-setup` and `/oh-my-droid:help` for namespace consistency
...
- **Project Renamed**: Project renamed to `oh-my-droid`
  - npm package remains `oh-my-droid`: `npx oh-my-droid install`
  - Plugin name is `oh-my-droid` for Factory Droid integration
...
- **Directory Structure**: `.sisyphus/` → `.omd/` (oh-my-droid)
  - State files now in `~/.factory/.omd/`
...
- **Slash Commands Updated**: Agent-referencing commands now use new names
  - `/oh-my-droid:plan` now uses `planner` agent (was `prometheus`)
  - `/oh-my-droid:review` now uses `critic` agent (was `momus`)
  - `/oh-my-droid:mnemosyne` → `/oh-my-droid:learner` for skill extraction
...
Real-time visualization of the Sisyphus orchestration system via Factory Droid's statusline.

### Added

- **Mnemosyne - Learned Skills** (`src/hooks/mnemosyne/`)
  - `/oh-my-droid:mnemosyne` command to extract reusable skills from conversations
  - Automatic skill injection based on trigger keywords in user messages
  - **Hybrid storage**: User-level (`~/.factory/skills/sisyphus-learned/`) + Project-level (`.sisyphus/skills/`)
  ...
  - Configuration system with `~/.factory/sisyphus/mnemosyne.json`
...
- **Sisyphus HUD Statusline** (`src/features/hud/`)
  ...
  - `/oh-my-droid:hud` command to configure display options
  - Auto-refresh every ~300ms during active sessions
  - Type-coded agent visualization with model tier colors

- **New Commands**
  - `/oh-my-droid:mnemosyne` - Extract learned skills from current conversation
  - `/oh-my-droid:hud [preset]` - Configure HUD display (minimal/focused/full)
...
- Renamed `/oh-my-droid:claudeception` to `/oh-my-droid:mnemosyne`
...
- **Help Command** (`commands/help.md`)
  - Comprehensive `/oh-my-droid:help` command to guide users on plugin usage
...
- **New Commands**
  - `/oh-my-droid:ralph-init <task>` - Scaffold a PRD from task description with auto-generated user stories
  - `/oh-my-droid:ultrawork-ralph <task>` - Maximum intensity mode with completion guarantee (ultrawork + ralph loop)
  - `/oh-my-droid:ultraqa <goal>` - Autonomous QA cycling workflow (test → verify → fix → repeat)
  - `/oh-my-droid:sisyphus-default` - Configure Sisyphus in local project `.factory/FACTORY.md`
  - `/oh-my-droid:sisyphus-default-global` - Configure Sisyphus globally in `~/.factory/FACTORY.md`
  - `/oh-my-droid:note <content>` - Save notes to notepad.md for compaction resilience
...
- **Ralph Loop Enhanced**
  - Auto-initializes PRD when user runs `/oh-my-droid:ralph-loop` without existing `prd.json`
...
- **Agent name prefixing** - Standardized all Task() calls to use `oh-my-droid:` prefix
...
- **Agent frontmatter** - Added missing `name` and `description` fields to all 11 agents
  - Per Factory Droid sub-agent specification requirements
...
### Added
- **QA-Tester Agent** (`src/droids/qa-tester.ts`)
...
- **Merged sisyphus+orchestrator+ultrawork into default mode** - 80% behavior overlap consolidated
  ...
  - `/oh-my-droid:orchestrator` command deprecated (use default mode or `/oh-my-droid:ultrawork`)
...
### Added
- **Enhanced Hook Enforcement System** - Stronger Sisyphus behavior enforcement beyond FACTORY.md
...
- PostToolUse hook tracks session statistics in `~/.factory/.session-stats.json`
...
- **Factory Droid Native Hooks Integration**
...
- State files: `.sisyphus/ultrawork-state.json`, `~/.factory/ultrawork-state.json`
...
- **SDK adaptation**
  - Converted all oh-my-opencode SDK patterns to Factory Droid SDK:
    - `sisyphus_task()` → `Task(subagent_type=...)`
    - `background_output()` → `TaskOutput()`
    - References to OhMyOpenCode → Oh-My-Droid-Sisyphus

### Verified
- All 6 builtin skills install correctly to `~/.factory/skills/`
...
- Architecture comparison documentation (OpenCode vs Factory Droid)
...
[2.6.0]: https://github.com/MeroZemory/oh-my-droid/compare/v2.5.0...v2.6.0
[2.0.1]: https://github.com/MeroZemory/oh-my-droid/compare/v2.0.0...v2.0.1
[1.11.0]: https://github.com/MeroZemory/oh-my-droid/compare/v1.10.0...v1.11.0
[1.10.0]: https://github.com/MeroZemory/oh-my-droid/compare/v1.9.0...v1.10.0
[1.9.0]: https://github.com/MeroZemory/oh-my-droid/compare/v1.8.0...v1.9.0
[1.8.0]: https://github.com/MeroZemory/oh-my-droid/compare/v1.7.0...v1.8.0
[1.7.0]: https://github.com/MeroZemory/oh-my-droid/releases/tag/v1.7.0
